...
namespace Api_ZserviceDesk.Models
{
    public class EmailModel
    {
        ...
        public ExceptionTracking SmtpSend(MailMessage user)
        {
            ...
            {
                smtp.Send(user);
                wl.LogWrite("UserName: " + zdesk_M_Smpt_Tbl.UserName);
                wl.LogWrite("Password: " + zdesk_M_Smpt_Tbl.Password);
                wl.LogWrite("SmptPort: " + zdesk_M_Smpt_Tbl.SmptPort);

                wl.LogWrite("Email sent successfully from SmtpSend function");
            ...
            {
                wl.LogWrite("Error in SmtpSend function");
                ExceptionTracking et = new ExceptionTracking();
                et.status = ((System.Net.WebException)ex.InnerException) != null ? ((System.Net.WebException)ex.InnerException).Status.ToString() : null;
                et.message = ex.Message;
                et.exception_message = ex.InnerException != null ? ex.InnerException.Message : null;
        ...
        {
            if (vendornotification.VendorEmail != "N/A")
            {
                using (MailMessage user = new MailMessage(zdesk_M_Smpt_Tbl.FromAddress, vendornotification.VendorEmail))
                {
                    ...
                    var result = SmtpSend(user);
                    zdesk_m_email_audit_tbl ea = new zdesk_m_email_audit_tbl();
                    ea.email_to = vendornotification.VendorEmail;
                    ea.subject = vendornotification.subject;
                    ea.status = result.is_sent;
                    ea.email_status = result.status;
                    ea.error_message = result.exception_message;
                    ea.exception_message = result.exception_message;
        ...
        public void SendIncidentStatusChange(TicketStatus ce)
        {
            using (MailMessage user = new MailMessage(zdesk_M_Smpt_Tbl.FromAddress, ce.ReceiptEmail))
            {
                ...
                {
                    myString = myString.Replace("$$divResolved$$", "<strong>For your information,our process requires a closure of any case that has no contact for 2 days. You can open a new case when you have more time to work on the issue and as always, there is not limit to the number of cases you can open.</strong><br /><br />");
                }
                else
                {
                    myString = myString.Replace("$$divResolved$$", "");
                }
        ...
        public void SendTicketAssignTechnician(TicketStatus ce)
        {
            ...
            {
                wl.LogWrite("Error in SendTicketAssignTechnician for Ticket " + ce.TicketId + " for " + ReceiptEmail + ": " + ex.Message);
            }
        }
        // Send Email
        ...
        public void SendEmailChangeStatus_SR(ServiceRequestStatus ce)
        {
            try
            {
                using (MailMessage user = new MailMessage(zdesk_M_Smpt_Tbl.FromAddress, ce.ReceiptEmail))
                {
                    ...
                    if (ce.ApproverLevel == "2")
                    {
                        myString = myString.Replace("$$CommentsApprover1$$", ce.StatusComments);

                        myString = myString.Replace("$$dispApprover1$$", "''");
                        myString = myString.Replace("$$dispApprover2$$", "display:none;");
                        myString = myString.Replace("$$dispApprover3$$", "display:none;");
                    }
                    else if (ce.ApproverLevel == "3")
                    {
                        myString = myString.Replace("$$CommentsApprover1$$", ce.StatusComments);
                        myString = myString.Replace("$$CommentsApprover2$$", ce.StatusComments1);

                        myString = myString.Replace("$$dispApprover1$$", "''");
                        myString = myString.Replace("$$dispApprover2$$", "''");
                        myString = myString.Replace("$$dispApprover3$$", "display:none;");
                    }
                    else
                    {
                        myString = myString.Replace("$$dispApprover1$$", "display:none;");
                        myString = myString.Replace("$$dispApprover2$$", "display:none;");
                        myString = myString.Replace("$$dispApprover3$$", "display:none;");
                    }
                    ...
                    if (result.is_sent == 1)
                    {
                        wl.LogWrite("Email sent successfully for Service Request: " + ce.ServiceRequestId);
                    }
            ...
            {
                wl.LogWrite("Error in SendEmailChangeStatus_SR function:" + ex.Message);
            }
        }
        ...
        public void SendEmailTicketReplyStatusNew(TicketReplyStatus ce, HttpPostedFile file)
        {
            ...
            using (MailMessage user = new MailMessage(zdesk_M_Smpt_Tbl.FromAddress, ce.ReceiptEmail))
            {
                ...
                string myString = "";
                myString = t.Template;
                myString = myString.Replace("$$RequestID$$", ce.TicketId);
                myString = myString.Replace("$$Description$$", ce.Description);
                if (!string.IsNullOrEmpty(ce.url))
                {
                    myString = myString.Replace("$$Attachment$$", "<a href='" + ce.url + "'>View</a>");
                }
                else
                {
                    myString = myString.Replace("$$Attachment$$", "N/A");
                }
                myString = BindOrgInfo(myString);
                user.Subject = t.subject;
                user.Body = myString.ToString();
                ...
                if (ce.CCEmail != null && ce.CCEmail != "" && ce.CCEmail != "null")
                {
                    string[] CCId = ce.CCEmail.Split(',');
                    foreach (string CCEmail in CCId)
                    {
        ...
        public void SendEmailServiceRequestReplyStatus(ServiceRequestReplyStatus ce)
        {
            using (MailMessage user = new MailMessage(zdesk_M_Smpt_Tbl.FromAddress, ce.ReceiptEmail))
            {
                ...
                user.Body = myString.ToString();
                user.IsBodyHtml = true;
                if (ce.CCEmail != null && ce.CCEmail.EndsWith(","))
                {
                    ce.CCEmail = ce.CCEmail.TrimEnd(',');
                }
                if (ce.CCEmail != null && ce.CCEmail != "")
        ...
        // Send Email for Purchase Order when purchase order Created 3 approval 
        public void SendEmailChangePurchaseOrderStatus(PurchaseOrderStatus ce)
        {
            using (MailMessage user = new MailMessage(zdesk_M_Smpt_Tbl.FromAddress, ce.ReceiptEmail))
            {
                EmailTemplateModel t = new EmailTemplateModel();
                ...
                myString = t.Template;
                myString = myString.Replace("$$ApproverLevel$$", EncryptDecrypt.EncryptUrl(ce.ApproverLevel));
                myString = myString.Replace("$$PurchaseOrderID$$", ce.PurchaseOrderID);
                myString = myString.Replace("$$ID$$", EncryptDecrypt.EncryptUrl(ce.ID));
                myString = myString.Replace("$$POName$$", ce.POName);
                //////myString = myString.Replace("$$UserName$$", ce.Name);
                myString = myString.Replace("$$ApproverName$$", ce.ApproverName);
                user.Subject = t.subject;
                myString = BindOrgInfo(myString);
