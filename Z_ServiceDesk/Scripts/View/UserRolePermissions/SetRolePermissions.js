$(document).ready(function () {
    ...
    if (UserSession != null) {
        SetUserRolePermissionsRequest(UserSession[0].UserID);
    }
...
function ConvertJsontoDataTable(data) {
    ...
    for (var i = 0; i < data.length; i++) {
        ...
        tr.append("<td id=tblRP_td_View_Name_" + i + " style='padding:10px'>" + PermissionFlag("tblRP_lbl_View_Name_" + i, data[i].View_Name) + "<div id=divView_" + i + " class='dis-none' style='margin-left:5px;'><input id=cbkView_" + i + " type='checkbox' /><label for='View' style='margin-left:2px;'>View</label></div></td>");
        tr.append("<td id=tblRP_td_Add_Name_" + i + " style='padding:10px'>" + PermissionFlag("tblRP_lbl_Add_Name_" + i, data[i].Add_Name) + "</label><div id=divAdd_" + i + " class='dis-none' style='margin-left:5px;'><input id=cbkAdd_" + i + " type='checkbox' /><label for='Add' style='margin-left:2px;'>Add</label></div></td>");
        tr.append("<td id=tblRP_td_Edit_Name_" + i + " style='padding:10px'>" + PermissionFlag("tblRP_lbl_Edit_Name_" + i, data[i].Edit_Name) + "</label><div id=divEdit_" + i + " class='dis-none' style='margin-left:5px;'><input id=cbkEdit_" + i + " type='checkbox' /><label for='Edit' style='margin-left:2px;'>Edit</label></div></td>");
        tr.append("<td id=tblRP_td_Delete_Name_" + i + " style='padding:10px'>" + PermissionFlag("tblRP_lbl_Delete_Name_" + i, data[i].Delete_Name) + "</label><div id=divDelete_" + i + " class='dis-none' style='margin-left:5px;'><input id=cbkDelete_" + i + " type='checkbox' /><label for='Delete' style='margin-left:2px;'>Delete</label></div></td>");
        ...
        if (data[i].IsEnabledForEdit_Name == 'Yes') {
            tr.append("<td id=tblRP_td_Action_" + i + "><div class=' pull-left'><button id='btnEdit_" + i + "' type='button' class='btn btn-sm btn-soft-success btn-circle mr-2' onclick='rowEdit(this.id);'><i class='dripicons-pencil'></i></button><button id='btnUpdate_" + i + "' type='button' class='btn btn-sm btn-soft-purple mr-2 btn-circle' style='display:none;' onclick='rowUpdate(this.id);'><i class='dripicons-checkmark'></i></button><button id='btnCancel_" + i + "' type='button' class='btn btn-sm btn-soft-info btn-circle' style='display:none;' onclick='rowCancel(this.id);'><i class='dripicons-cross' aria-hidden='true'></i></button></div></td>");
        }

        if (data[i].IsEnabledForEdit_Name == 'No') {
            tr.append("<td id=tblRP_td_Action_" + i + "><div class=' pull-left'></div></td>");
        }
...
function PermissionFlag(ID, PermissionFlagName) {
    
    var Value = '';

    if (PermissionFlagName == 'Yes') {
        Value = '<label id=' + ID + ' style="color:#2ddab5!important;">' + PermissionFlagName + '</label>';
        ////Value = ' title="Severity - 1" style="color:#ea2d2d !important;margin-right:5px;" data-original-title="Critical priority"';
    }

    if (PermissionFlagName == 'No') {
        Value = '<label id=' + ID + ' style="color:#ef4d56!important;">' + PermissionFlagName + '</label> ';
        ////Value = ' title="Severity - 2" style="color:orange !important;margin-right:5px;" data-original-title="High priority"';
    }
...
////////$('#ConfirmationModal.modal.fade').on('click', function () {
////////    RedirectPage('Users');
////////})
